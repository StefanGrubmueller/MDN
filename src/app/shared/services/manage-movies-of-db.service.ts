...
export class ManageMoviesOfDbService {
  ...
  public getAllMovies$(): Observable<MovieType[] | undefined> {
    return this._cachedAllMovies !== undefined &&
      this._cachedAllMovies.length > 0
      ? of(this._cachedAllMovies)
      : this.fetchAllMoviesFromDB$();
  }

  public getAllWatchedMovies$(): Observable<MovieType[] | undefined> {
    return this._cachedWatchedMovies !== undefined &&
      this._cachedWatchedMovies.length > 0
  ...
  public deleteMovie(id: string): void {
    this._cachedAllMovies = this._cachedAllMovies?.filter(
      (localMovie: MovieType) => {
        return id !== localMovie.id;
      },
    );
    this._cachedWatchedMovies = this._cachedWatchedMovies?.filter(
      (movie) => movie.id !== id,
    );
  ...
  public updateMovie(movie: MovieType): void {
    ...
    const index = this._cachedWatchedMovies.findIndex(
  ...
  private fetchAllMoviesFromDB$(): Observable<MovieType[] | undefined> {
    ...
    return from(this.db.collection(email).get()).pipe(
