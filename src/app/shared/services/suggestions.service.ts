...
export class SuggestionsService {
  ...
  loadSuggestions(numberOfSuggestions: number): Observable<Suggestion[]> {
    return this.manageMovieService.getAllMovies$().pipe(
      switchMap((movies: MovieType[] | undefined) => {
        const suggestedMovieIds: string[] = movies
          ? _.sample(movies, numberOfSuggestions).map((movie) => movie.id)
          : [];
        const suggestionObservables: Observable<any>[] = suggestedMovieIds.map(
          (movieId: string) => this.imdbService.getImdbMovieDetails(movieId),
        );
        return this.getSuggestionsFromObservables(
          suggestionObservables,
          movies ?? [],
        );
      }),
    );
  ...
  private getSuggestionsFromObservables(
    ...
    return forkJoin(observables).pipe(
      map((movieDetails: any[]) => {
        ...
        movieDetails.forEach((movieDetail: any) => {
          if (
            movieDetail?.suggestions?.relatedMoviePosterUrl &&
            movies?.filter(
              (movie) => movie.id === movieDetail.suggestions.suggestionId,
            ).length <= 0
          ) {
